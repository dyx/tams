  ...
  <el-dialog title="修改" width="790px" :close-on-click-modal="false" :close-on-press-escape="false" v-model="dialogVisible" :before-close="handleClose">
    <el-form ref="formRef" :model="form" :rules="rules" inline label-width="80px" class="tams-form-container">
      ...
      <el-form-item label="时间" prop="attendTime">
        <el-time-select :clearable="false" style="width: 210px;" v-model="form.attendTime" :picker-options="pickerOptions" :key="attendTimeSelectKey" @change="calcFinishTime" />
      </el-form-item>
      <el-form-item>
...
const search = () => {
  csStore.getCourseSchedulingById(props.id).then((res: any) => {
    if (res) {
      ...
      courseDuration.value = form.value.duration
      currentCourse.value = { id: res.courseId, name: res.courseName }
    }
  }).catch(() => {})
}
  ...
  formRef.value?.validate((valid) => {
    if (valid) {
      ...
      csStore.updateCourseSchedulingById(props.id, form.value).then(() => {
        submitBtnLoading.value = false; formRef.value?.resetFields()
        emit('on-success'); dialogVisible.value = false
      }).catch(() => {
        form.value.attendTime = dayjs(form.value.attendTime, 'HH:mm:ss').format('HH:mm')
        form.value.finishTime = dayjs(form.value.finishTime, 'HH:mm:ss').format('HH:mm')
